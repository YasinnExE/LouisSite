...
app.post("/sharing", async (req, res) => {
  ...
  if (cooldown.get(req.user.id) && cooldown.get(req.user.id).count >= 20) return error(res, 429, "10 dakika içerisinde en fazla 20 kod paylaşabilirsin!");
  ...
  else if (code.rank === "altyapi") color = "";
  ...
  `)
  .setColor(color)//oldu abone botu /hi /aloooo //alooo//
  channel.send(embed);
...
app.get("/altyapi/:codeID", async (req, res) => {
  ...
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.altyapıRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!");
  const codeData = require("./src/schemas/code");
...
app.get("/delete/:rank/:id", async (req, res) => {
  ...
  if (!member.roles.cache.get("888024286357520386")) return error(res, 401, "Bu sayfaya girmek için yetkin bulunmuyor!");
